...
export default function SelectMissingWord(props) {    
    ...
    return (
        <View style={SelectMissingWordStyles.container}>
            {
                props.data.words && props.data.words.map((itm) => {
                    return <Pressable
                                key={itm.id}
                                onPress={() => checkAnswer(itm)}
                                style={itm.id == pressedID ? SelectMissingWordStyles.activeBtn : SelectMissingWordStyles.inactiveBtn} >
                                <Text style={SelectMissingWordStyles.itemText}>{itm.word}</Text>
                            </Pressable>
                })
            }
        </View>
...
const SelectMissingWordStyles = StyleSheet.create({
    container: {
        display: 'flex',
        width: "100%",
        alignItems: 'center',
